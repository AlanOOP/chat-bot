...
import os
import json
from utils.embeddings import convert_to_embeddings
...
r = input('Desea Crear vectores? (Y/N): ')

# os.system('cls')
if r == 'Y':
    ingest_response = convert_to_embeddings(openai_api_key=openai_api_key, pinecone_api_key=pinecone_api_key,
    ...
    if use_pinecone:
        print('Usando vectores ya ingresados en Pinecone.')
    else:
        print('Using already ingested vectors at ./vectorstore.')
else:
    print('No se proporciona ningún método, pasando')
    pass
...
def chat_loop():
    chat_history = []
    while True:
        query = input("Por favor, ingrese su pregunta (Escriba 'exit' para salir): ")
        if query.lower() == 'exit':
            break
